export const PersonalFinanceAnalyzer = {
  analyzeDomain(data, context) {
    ...
    const trend = Object.entries(agg).sort(([a],[b])=>a>b?1:-1).map(([month, amount])=>({ month, amount }));
    const cat = {};
    rows.forEach(r => {
      const name = r[catKey];
      if (!name) return;
    ...
    const ranking = Object.entries(cat).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([name, amount])=>({ name, amount }));
    return { metrics: { primary: metric }, insights: { summary: `Personal finance detected. ${rows.length} rows.` }, visuals: { trend, ranking } };
  }
};
