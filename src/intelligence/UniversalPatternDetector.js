...
export class UniversalPatternDetector {
  static analyze(data, sampleSize = 2000) {
    ...
    const top = domains.sort((a, b) => b.score - a.score)[0] || {
      domain: "generic",
      score: 0.5,
    };
  ...
  static roleHintsFromHeader(h) {
    ...
    const map = {
      ...
      personal_money:
        /(expense|category|budget|wallet|account|txn|transaction)/,
  ...
  static inferType(rows, key) {
    ...
    const num = values.filter(
      (v) => !isNaN(parseFloat(String(v).replace(/,/g, ""))),
    );
    const bool = values.filter((v) =>
      /^(true|false|yes|no|1|0|y|n)$/i.test(String(v)),
    );
  ...
  static findDate(info) {
    return (
      info.find((c) => c.type === "date" || c.roleHints.includes("date"))
        ?.name || null
    );
  }

  static detectMetrics(info) {
    const candidates = info.filter(
      (c) => c.type === "numeric" && c.uniqueness < 90,
    );
    const financeFirst = candidates.sort((a, b) => {
      const as =
        a.roleHints.includes("finance_rev") ||
        a.roleHints.includes("finance_cost")
          ? 1
          : 0;
      const bs =
        b.roleHints.includes("finance_rev") ||
        b.roleHints.includes("finance_cost")
          ? 1
          : 0;
  ...
  static detectEntities(info) {
    const entities = {};
    const find = (roles) =>
      info.find((c) => roles.some((r) => c.roleHints.includes(r)))?.name;
    entities.customer = find(["customer"]);
    entities.product = find(["product"]);
    entities.location = find(["geo"]);
    entities.person = find(["hr_person"]);
    entities.category =
      find(["personal_money", "product"]) ||
      info.find((c) => c.type === "text" && c.uniqueness < 60)?.name;
    return entities;
  }
    ...
    const candidates = [
      {
        ...
        score:
          score([has("personal_money"), manyNumeric, hasDate]) +
          (has("finance_cost") || has("finance_rev") ? 0.5 : 0),
      },
      {
        domain: "finance",
        score:
          score([
            has("finance_rev") || has("finance_cost"),
            manyNumeric,
            hasDate,
          ]) + (has("customer") ? 0.25 : 0),
      },
      {
        domain: "inventory",
        score:
          score([has("inventory"), has("product"), manyNumeric]) +
          (hasDate ? 0.25 : 0),
      },
      ...
      {
        domain: "academic",
        score: score([has("academic"), manyNumeric, hasDate]),
      },
      {
        domain: "business_sales",
        score:
          score([has("customer"), has("product"), has("finance_rev")]) +
          (hasDate ? 0.25 : 0),
      },
      {
        domain: "generic",
        score: 1 + (manyNumeric ? 0.25 : 0) + (hasDate ? 0.25 : 0),
      },
    ];
    const max = Math.max(...candidates.map((c) => c.score)) || 1;
    return candidates.map((c) => ({
      domain: c.domain,
      score: Math.min(1, c.score / (max + 0.0001)),
    }));
  }
  ...
  static suggestAnalyses(domain, info, metrics) {
    ...
    switch (domain) {
      case "personal_finance":
        suggestions.add("budget_vs_expense").add("category_trends");
        break;
      case "finance":
        suggestions.add("ratio_analysis").add("cashflow_forecast");
        break;
      case "inventory":
        suggestions.add("stock_turnover").add("reorder_points");
        break;
      case "hr":
        suggestions.add("salary_bands").add("headcount_trends");
        break;
      case "health":
        suggestions.add("habit_consistency").add("progress_tracking");
        break;
      case "academic":
        suggestions.add("grade_trends").add("subject_performance");
        break;
      default:
        suggestions.add("generic_summary");
    }
