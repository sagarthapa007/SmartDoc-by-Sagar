...
export default function ExportCenter() {
  ...
  if (!ds)
    return (
      <div className="opacity-70 text-sm">Upload a dataset to export.</div>
    );
  return (
    <div className="grid md:grid-cols-2 gap-3">
      <div className="card">
        ...
        <div className="grid gap-2 text-sm">
          ...
          <button className="kbd" onClick={() => handleExport(fmt, ds)}>
            Export
          </button>
        </div>
      </div>
      <div className="card">
        ...
        <div className="text-sm opacity-70">
          PDF/HTML/PPT/Excel are wired with graceful fallbacks. CSV works
          out-of-the-box.
        </div>
      </div>
    </div>
...
async function handleExport(fmt, ds) {
  if (fmt === "csv") {
    ...
    try {
      ...
      const doc = new jsPDF();
      doc.text("SmartDoc Report", 20, 20);
      doc.text("Rows: " + ds.rows.length, 20, 30);
      doc.save("smartdoc.pdf");
    } catch (e) {
      alert("Install PDF engine: npm i jspdf");
    }
  } else if (fmt === "pptx") {
    try {
      ...
      slide.addText("SmartDoc Summary", { x: 1, y: 0.5, fontSize: 24 });
      slide.addText(`Rows: ${ds.rows.length}  Cols: ${ds.headers.length}`, {
        x: 1,
        y: 1.2,
        fontSize: 14,
      });
      await pptx.writeFile({ fileName: "smartdoc.pptx" });
    } catch (e) {
      alert("Install PPT engine: npm i pptxgenjs");
    }
  } else if (fmt === "xlsx") {
    try {
      ...
      XLSX.writeFile(wb, "smartdoc.xlsx");
    } catch (e) {
      alert("XLSX export requires sheetjs already installed (it is).");
    }
  }
}
function renderHTML(ds) {
  ...
  <tbody>${ds.rows
    ...
    .map(
      (r) =>
        `<tr>${ds.headers.map((h) => `<td>${String(r[h] ?? "")}</td>`).join("")}</tr>`,
    )
    .join("")}</tbody></table>
