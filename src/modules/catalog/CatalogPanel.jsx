  ...
  return (
    <div className="grid gap-3">
      <div className="text-sm opacity-70">
        Define and manage uploaded files. Set roles (Dataset / Document /
        Ignore) and add notes.
      </div>
      <div className="grid md:grid-cols-2 gap-3">
        {files.length === 0 && (
          <div className="opacity-70 text-sm">No files uploaded yet.</div>
        )}
        {files.map((f, idx) => (
          <div key={idx} className="border rounded-xl p-3">
            ...
            <div className="text-xs opacity-70 mt-1">
              Size: {(f.meta?.size || 0).toLocaleString()} bytes
            </div>
            {f.sheetName && (
              <div className="text-xs opacity-70">Sheet: {f.sheetName}</div>
            )}
          </div>
        ))}
      </div>
        ...
        <div className="border rounded-xl p-3">
          ...
          {ds && (
            <div className="text-xs opacity-70">
              Headers: {ds.headers?.length || 0}, Rows: {ds.rows?.length || 0}
            </div>
          )}
        </div>
        <div className="border rounded-xl p-3">
          ...
          {doc && (
            <div className="text-xs opacity-70">
              Excerpt len: {doc.excerpt?.length || 0}
            </div>
          )}
        </div>
      </div>
