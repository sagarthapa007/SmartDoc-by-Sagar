...
export default function Heatmap({ headers = [], rows = [] }) {
  const numericHeaders = useMemo(() => {
    return headers.filter(h => rows.some(r => !isNaN(parseFloat(String(r?.[h]).replace(/,/g, "")))));
  }, [headers, rows]);
  ...
  if (!numericHeaders.length) {
    return <div className="text-xs text-gray-500">No numeric columns for correlation heatmap.</div>;
  }
    ...
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-50">
      ...
      <svg width="100%" viewBox={`0 0 ${sizeX} ${sizeY}`} role="img">
        ...
        {numericHeaders.map((h, i) => (
          <text key={`ylab-${i}`} x={padLeft - 8} y={padTop + i*cell + cell*0.65} fontSize="10" textAnchor="end" fill="#374151">
            {h}
          </text>
        ))}
        {/* X labels */}
        {numericHeaders.map((h, j) => (
          <text key={`xlab-${j}`} x={padLeft + j*cell + cell*0.5} y={14} fontSize="10" textAnchor="middle" fill="#374151" transform={`rotate(-30 ${padLeft + j*cell + cell*0.5},14)`}>
            {h}
          </text>
        ))}
        {/* Cells */}
        {matrix.map((row, i) =>
          row.map((v, j) => (
            <g key={`c-${i}-${j}`}>
              ...
              {i===j && (
                <text x={padLeft + j*cell + cell/2} y={padTop + i*cell + cell/2 + 3} fontSize="9" textAnchor="middle" fill="#111827">1.00</text>
              )}
            </g>
          ))
        )}
        {/* Border */}
        <rect x={padLeft-1} y={padTop-1} width={cell*numericHeaders.length+2} height={cell*numericHeaders.length+2} fill="none" stroke="#e5e7eb"/>
      </svg>
      <div className="text-[10px] text-gray-500 mt-2">Tip: hover a square to see exact correlation.</div>
    </div>
  );
}
