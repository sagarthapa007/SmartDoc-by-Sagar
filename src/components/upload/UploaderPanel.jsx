...
import { Upload as UploadIcon, Loader, AlertCircle, CheckCircle } from "lucide-react";
...
export default function Upload() {
  ...
  const handleFile = (e) => {
    ...
    if (f) { setFile(f); setReport(null); setError(""); }
  };
  ...
  const confirmAndAnalyze = async (report) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}/api/analyze`, { upload_id: report.upload_id });
      setDataset(res.data);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 flex flex-col items-center p-6">
      <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} className="text-center mb-8">
        ...
        <p className="text-gray-600 text-sm max-w-xl mx-auto">
          Upload CSV, Excel, JSON, DOCX, or PDF. SmartDoc will classify and prepare it for AI analysis.
        </p>
      </motion.div>

      <div className={`w-full max-w-3xl border-2 border-dashed rounded-2xl p-10 flex flex-col items-center justify-center ${loading?'border-blue-400 bg-blue-50':'border-gray-300 bg-white hover:border-blue-400 hover:bg-blue-50/20'}`}>
        <input id="fileInput" type="file" accept=".csv,.xlsx,.xls,.json,.pdf,.docx"
               onChange={handleFile} className="hidden"/>
        <label htmlFor="fileInput" className="cursor-pointer flex flex-col items-center">
          {loading ? <Loader className="w-14 h-14 text-blue-600 animate-spin mb-3"/> :
                     <UploadIcon className="w-14 h-14 text-blue-600 mb-3"/>}
          <span className="text-lg font-semibold text-gray-800 mb-1">{file?file.name:"Drag or click to upload"}</span>
          <span className="text-sm text-gray-500">Accepted: CSV, XLSX, JSON, DOCX, PDF</span>
        </label>
