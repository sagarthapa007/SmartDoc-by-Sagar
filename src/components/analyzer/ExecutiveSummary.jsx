...
export default function ExecutiveSummary({ kpis = {}, trends = {}, summary = {} }) {
  if (!kpis?.primaryMetric) return null;

  const items = [
    { label: "Primary Metric", value: String(kpis.primaryMetric) },
    { label: "Growth", value: trends?.growth != null ? `${(trends.growth * 100).toFixed(1)}%` : "—" },
    { label: "Direction", value: trends?.direction || "—" },
    { label: "Confidence", value: kpis?.confidence != null ? `${Math.round(kpis.confidence * 100)}%` : "—" }
  ];
  ...
  return (
    <section className="p-4 bg-white rounded-2xl shadow-sm mb-4 border border-gray-50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
        {items.map((it) => <KpiCard key={it.label} title={it.label} value={it.value} />)}
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {basic.map((it) => <MiniStat key={it.label} title={it.label} value={it.value} />)}
      </div>
    </section>
  );
}
