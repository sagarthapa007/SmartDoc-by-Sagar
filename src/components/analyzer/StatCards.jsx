...
export default function StatCards({ headers = [], rows = [] }) {
    ...
    const missing = (() => {
      ...
      for (let i = 0; i < Math.min(rowCount, 500); i++) {
        ...
        headers.forEach((h) => {
          total += 1;
          const v = r?.[h];
          if (v === null || v === undefined || v === "" || (typeof v === "number" && Number.isNaN(v))) miss += 1;
        });
      }
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
      {stats.map((s) => (
        <div key={s.label} className="rounded-lg border border-[var(--border)] bg-[var(--card)] px-3 py-3 sm:px-4 sm:py-4">
          <div className="text-[10px] sm:text-xs text-[var(--text-muted)] uppercase tracking-wide">{s.label}</div>
