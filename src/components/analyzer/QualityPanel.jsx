...
export default function QualityPanel({ quality }) {
  ...
  return (
    <div className="card">
      ...
      {totalIssues === 0 ? (
        <div className="text-sm text-[var(--text-tertiary)]">No obvious issues detected.</div>
      ) : (
        <div className="space-y-2">
          {Object.entries(missing).map(([col, count]) => (
            <div key={col} className="flex items-center justify-between border border-[var(--border)] rounded-lg px-3 py-2">
