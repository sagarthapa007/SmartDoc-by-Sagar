...
export default function Header() {
  ...
  // Memoized menu configuration for better performance
  const menu = useMemo(() => [
    { label: "Upload", path: "/", icon: <Database size={16} /> },
  ...
  // Active menu item style constants for maintainability
  const activeStyles = "bg-[var(--brand1)]/15 text-[var(--brand1)] border border-[var(--brand1)]/30 shadow-sm";
  const inactiveStyles = "text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--muted)]";
  ...
  return (
    <header className="sticky top-0 z-50 bg-[var(--background)]/95 backdrop-blur-lg border-b border-[var(--border)] shadow-sm supports-[backdrop-filter]:bg-[var(--background)]/80">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3 flex items-center justify-between gap-3 flex-nowrap">
        ...
        {/* === CENTER: DESKTOP MENU === */}
        <nav 
          className="hidden md:flex items-center gap-1 flex-1 justify-center relative z-10"
          ...
          {menu.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[var(--brand1)] focus:ring-offset-2 ${
        ...
        <div className="hidden md:flex items-center gap-2 flex-shrink-0">
          <div 
            className="flex items-center gap-2 border border-[var(--border)] rounded-lg px-2 py-1.5 bg-[var(--surface)] focus-within:ring-1 focus-within:ring-[var(--brand1)] focus-within:border-[var(--brand1)] transition-all duration-200"
            role="search"
          >
          ...
          <button
            className="kbd hover:scale-105 transition-transform"
        ...
        {/* === MOBILE MENU BUTTON === */}
        <button
          className="md:hidden kbd hover:scale-105 transition-transform"
      ...
      {mobileOpen && (
        <div 
          id="mobile-menu"
          className="md:hidden border-t border-[var(--border)] bg-[var(--background)]/95 backdrop-blur-md shadow-lg animate-slideDown supports-[backdrop-filter]:bg-[var(--background)]/90"
          role="dialog"
        ...
        >
          <nav className="flex flex-col px-4 py-3 gap-1" aria-label="Mobile navigation">
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-[var(--border)]">
              <button
                onClick={handleSettingsClick}
                className="flex items-center gap-2 text-sm opacity-80 hover:opacity-100 transition-opacity py-1"
                aria-label="Open settings"
              >
