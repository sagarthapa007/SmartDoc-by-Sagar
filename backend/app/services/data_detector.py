...
class DataDetector:
    def detect(
        ...
        # naive mapping
        for k in tokens:
            if k in {"amount", "revenue", "sales"}:
                detected_columns["revenue"] = k
            if k in {"customer", "client", "name"}:
                detected_columns["customer"] = k
            if k in {"date", "order_date", "doj"}:
                detected_columns["date"] = k
