...
def extract_column_patterns(headers):
    ...
    patterns = {
        ...
        'has_amounts': any(re.search(r'\b(amount|price|cost|value|total|sum)\b', h.lower()) for h in headers),
        'has_names': any(re.search(r'\b(name|title|description|note|remark)\b', h.lower()) for h in headers),
        'has_status': any(re.search(r'\b(status|state|flag|active|completed)\b', h.lower()) for h in headers),
    }
    return patterns
    ...
    if cache_file and os.path.exists(cache_file):
        try:
            ...
            if cached_data.get('timestamp') and \
               (datetime.now() - cached_data['timestamp']).total_seconds() < 3600:
...
def detect_data_type(headers, sample_rows=None, include_detailed_info=False):
    ...
    if sample_rows:
        try:
            sample_text = " ".join(
                str(value) for row in sample_rows[:100]  # Limit to first 100 rows for performance
    ...
    if MODEL_INFO['model'] and MODEL_INFO['vectorizer']:
        try:
            ...
            alternatives = [
                ...
                for i in prob_indices[1:4] if probs[i] > 0.1  # Only include meaningful alternatives
            ]
    ...
    if result["method"] == "unknown":
        try:
            ...
            result.update({
                ...
                "method": "keyword",
                "alternatives": [{"type": alt[0], "confidence": round(alt[1], 2)} for alt in alternatives],
...
def suggest_analyses(domain):
    ...
    suggestions = {
        "sales": [
            "revenue_trends", "customer_segmentation", "conversion_funnel",
            "sales_performance", "regional_analysis", "product_performance"
        ],
        "hr": [
            "headcount_analysis", "salary_distribution", "attendance_trends",
            "performance_reviews", "turnover_analysis", "department_breakdown"
        ],
        "finance": [
            "pnl_summary", "expense_breakdown", "cashflow_forecast",
            "budget_vs_actual", "financial_ratios", "trend_analysis"
        ],
        "marketing": [
            "campaign_performance", "roi_tracking", "lead_conversion",
            "channel_effectiveness", "audience_analysis", "engagement_metrics"
        ],
        "procurement": [
            "supplier_performance", "purchase_trends", "cost_analysis",
            "vendor_comparison", "inventory_turnover", "savings_analysis"
        ],
        "factory": [
            "machine_utilization", "downtime_analysis", "production_efficiency",
            "quality_metrics", "shift_comparison", "maintenance_scheduling"
        ],
        "manufacturing": [
            "batch_yield", "defect_trends", "production_volume",
            "process_optimization", "quality_control", "throughput_analysis"
        ],
        "personal_expense": [
            "spending_pattern", "budget_comparison", "category_breakdown",
            "trend_analysis", "savings_rate", "expense_forecasting"
        ],
        "it": [
            "ticket_resolution_time", "device_uptime", "system_performance",
            "incident_trends", "user_satisfaction", "resource_utilization"
        ],
        "admin": [
            "policy_compliance", "asset_audit", "vendor_management",
            "contract_tracking", "facility_utilization", "cost_optimization"
        ],
        "personal_life": [
            "habit_trends", "wellness_score", "goal_progress",
            "mood_correlation", "routine_analysis", "improvement_tracking"
        ],
        "healthcare": [
            "patient_outcomes", "treatment_effectiveness", "resource_utilization",
            "appointment_analysis", "medication_tracking", "clinical_metrics"
        ],
        "education": [
            "student_performance", "attendance_correlation", "grade_distribution",
            "course_effectiveness", "learning_progress", "assessment_analysis"
        ],
        "ecommerce": [
            "sales_performance", "customer_behavior", "inventory_optimization",
            "conversion_analysis", "product_reviews", "shipping_efficiency"
        ],
        "generic": [
            "basic_summary", "pattern_detection", "outlier_identification",
            "correlation_analysis", "trend_detection", "data_quality_check"
        ]
    }
