...
def _recommendations(kind: str, scrutiny: Dict[str, Any]) -> List[str]:
    ...
    if kind == "large_tabular":
        ...
        if high_missing_cols:
            recs.append(f"Consider imputation or removal for columns with >20% missing values: {', '.join(high_missing_cols[:3])}")
...
def _analyze_data_quality(quality: Dict[str, Any], schema: List[Dict]) -> Tuple[List[str], List[Dict[str, Any]]]:
    ...
    if type_counts:
        insights.append(f"Data types: {', '.join(f'{k}({v})' for k, v in type_counts.most_common())}")
...
def analyze_from_scrutiny(upload_id: str, scrutiny: Dict[str, Any]) -> Dict[str, Any]:
    ...
    insights.append(f"File: {file_type.upper()} | Type: {kind.replace('_', ' ').title()} | ID: {upload_id}")
    ...
    if kind in ("long_document", "document", "long_text", "text", "short_text"):
        ...
        insights.append(f"Text analysis: {word_count} words, {unique_words} unique ({round(unique_words/word_count*100 if word_count else 0, 1)}% diversity)")
        ...
        insights.append(f"Sentiment: {sentiment_result['tone']} (score: {sentiment_result['score']}, confidence: {sentiment_result['confidence']})")
    ...
    if kind in ("tabular", "large_tabular"):
        ...
        if type_summary:
            type_insight = "Column types: " + ", ".join([f"{k}({v})" for k, v in type_summary.most_common()])
            insights.append(type_insight)
        
        # Sample data distribution for categorical columns
        cat_columns = [c["name"] for c in schema if c.get("type") in ("categorical", "string") and c.get("name")]
        if preview_rows and cat_columns:
            # Analyze first 2 categorical columns
            for col_name in cat_columns[:2]:
                values = [str(row.get(col_name, "")) for row in preview_rows if col_name in row and row[col_name] is not None]
                if values:
                    freq = Counter(values)
    ...
    # ----- Enhanced Summary -----
    if kind in ("tabular", "large_tabular"):
        quality_issues = sum(1 for pct in quality.get("missing_pct", {}).values() if (pct or 0) > 0.1)
