...
def train_and_save_model(data, model_path, training_data_path=None):
    ...
    print(
        f"⚠️  Low-confidence predictions (<{CONFIDENCE_THRESHOLD}): {low_confidence_count}/{len(yte)}"
    )
    ...
    try:
        ...
        if len(y) >= 20:  # Only run CV if we have enough data
            ...
            print(
                f"🔄 Cross-validation Accuracy: {np.mean(cv_scores):.3f} (+/- {np.std(cv_scores) * 2:.3f})"
            )
    ...
    if return_alternatives > 0 and len(classes) > 1:
        ...
        for i, (prob, cls) in enumerate(prob_class_pairs[1 : return_alternatives + 1], 1):
            if prob > 0.05:  # Only include meaningful alternatives
                alternative_predictions.append({"type": cls, "confidence": prob, "rank": i})
...
def load_model(model_path):
    ...
    try:
        ...
        print(
            f"📊 Model info: {model_data['training_info']['training_samples']} training samples, "
...
def interactive_test_mode(model_data):
    ...
    while True:
        try:
            ...
            print(
                f"   Status: {'HIGH confidence' if result['is_high_confidence'] else 'LOW confidence'}"
            )
