...
def summarize_schema(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for c in df.columns:
        s = df[c]
        t = _infer_type(s)
        entry = {"name": c, "type": t, "non_null": int(s.notna().sum()), "missing": int(s.isna().sum())}
        if t == "numeric":
            entry["min"] = float(pd.to_numeric(s, errors="coerce").min(skipna=True)) if len(s) else None
            entry["max"] = float(pd.to_numeric(s, errors="coerce").max(skipna=True)) if len(s) else None
...
def quality_report(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in df.select_dtypes(include=["number"]).columns:
        ...
        if idxs:
            outliers[col] = [{"row": int(i), "value": float(df.loc[i, col]), "z": float(z.loc[i])} for i in idxs[:100]]
    return {
        "duplicates": {"count": int(len(duplicates)), "rows": duplicates.head(50).to_dict(orient="records")},
        "missing": missing,
        "outliers": outliers,
    }

def correlations_for(df: pd.DataFrame, target: Optional[str] = None, threshold: float = 0.5) -> Dict[str, Any]:
