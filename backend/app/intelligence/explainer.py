    ...
    if "why" in q and ("drop" in q or "decrease" in q):
        if numeric_cols and date_cols:
            ...
            if changes:
                ...
                notes.append(
                    {
                        "text": f"Largest relative drop found in {worst[0]}: {worst[1]}% vs previous period.",
                        "detail": "MoM on first numeric column.",
                    }
                )
        else:
        ...
        dim_col = next(
            (h for h in headers if h.lower() in ("customer", "product", "category")), None
        )
        if val_col and dim_col:
            ...
            notes.append(
                {
                    "text": f"Top contributors by {dim_col}:",
                    "items": [{"name": k, "total": v} for k, v in ranked],
                }
            )
        else:
        ...
        notes.append(
            {"text": "No specific heuristic matched the question; try Explore or refine question."}
        )
